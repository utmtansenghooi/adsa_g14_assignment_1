...
def build_inventory_bst(key_type: InventoryKeyType = InventoryKeyType.PRODUCT_ID, class_type: Type[KeyType] = str, entries: int = -1) -> BinarySearchTree:
    ...
    # Shuffle product records to avoid sorted insertion
    random.shuffle(product_records)
...
def build_inventory_improved_splay_tree(key_type: InventoryKeyType = InventoryKeyType.PRODUCT_ID, class_type: Type[KeyType] = str, entries: int = -1) -> ImprovedSplayTree:
    """Build an improved splay tree of products from inventory data.
    
    Returns:
        ImprovedSplayTree with product_id or name as keys and Product objects as values
    """
    ...
    for record in product_records:
        ...
        node = ImprovedSplayNode(key=node_key, value=product)
        splay_tree.insert(node.key, node.value)

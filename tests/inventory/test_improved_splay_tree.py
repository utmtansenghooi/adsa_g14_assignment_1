...
def test_splay_tree_basic_operations():
    def _genSplay() -> ImprovedSplayTree[int]:
        t = ImprovedSplayTree[int]()    #        15
        t.insert(5)                     #       /
        t.insert(3)                     #      10
        t.insert(10)                    #     /
        t.insert(4)                     #    4   
...
def test_splay_tree_product_insertion_product_id():
    t = ImprovedSplayTree[ImprovedSplayNode]()
    products = [
    ...
    nodes = [ImprovedSplayNode(key=p.product_id, value=p) for p in products]
    for node in nodes:
...
def test_splay_tree_product_insertion_name():
    ...
    nodes = [ImprovedSplayNode(key=p.name, value=p) for p in products]
    for node in nodes:
        t.insert(node.key, node.value)
